...
class TestTaskList(unittest.TestCase):
    ...
    def test_task_list(self, index, casesname, module, id):
        ...
        path = TestTaskList.datas.get_request_url(index)
        url = readConfig.ReadConfig.get_http('baseurl') + path
        except_data = TestTaskList.datas.get_expect_data(index)

        try:
            if TestTaskList.datas.get_request_method(index) == 'post':
            ...
            TestTaskList.result.set_actual_data(globals()['sheet_id'], index, str(response.json()))  # 将实际结果写入excel

        except Exception as e:
            globals()['result'] = '报错啦'
            TestTaskList.result.set_actual_data(globals()['sheet_id'], index, str(e))
            TestTaskList.result.set_pass_fail(globals()['sheet_id'], index, globals()['result'])  # 写入测试结果

        TestTaskList.result.set_pass_fail(globals()['sheet_id'], index, globals()['result'])  # 先写入测试结果为不通过
        result = result_assert.result_assert(response.text, response.status_code, except_data)  # 断言，判断接口状态和预期结果
        if result == 'pass':
            globals()['result'] = 'pass'
            TestTaskList.result.set_pass_fail(globals()['sheet_id'], index, globals()['result'])  # 更新为测试通过
