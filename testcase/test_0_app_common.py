...
class TestAppCommon(unittest.TestCase):
    ...
    def test_app_common(self, index, casesname, module, id):
        ...
        try:
            ...
            globals()['result'] = '报错啦'
            TestAppCommon.result.set_actual_data(globals()['sheet_id'], index, str(e))
            TestAppCommon.result.set_pass_fail(globals()['sheet_id'], index, globals()['result'])  # 写入测试结果

            TestAppCommon.result.set_pass_fail(globals()['sheet_id'], index, globals()['result'])  # 先写入测试结果为不通过
        result = result_assert.result_assert(response.text, response.status_code, except_data)  # 断言，判断接口状态和预期结果
        if result == 'pass':
            globals()['result'] = 'pass'
            TestAppCommon.result.set_pass_fail(globals()['sheet_id'], index, globals()['result'])  # 更新为测试通过
